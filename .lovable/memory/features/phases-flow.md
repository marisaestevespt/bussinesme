...
## Timeline Rules (on phases AND deliverables)
- `duration_days`: how many days the item lasts
- `duration_unit`: `dias_uteis` (business days) or `dias_corridos` (calendar days)
- `offset_days`: days after trigger reference point
- Phase `offset_trigger`: `inicio_projeto` or `fase_anterior`
- Deliverable `offset_trigger`: `inicio_fase` or `entrega_anterior`
- `planned_start` / `planned_end`: calculated dates on project_phases AND project_deliverables, editable per project
...
- Portal shows planned_start/planned_end dates for both phases and deliverables
