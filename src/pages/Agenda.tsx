...
function AddToCalendarButtons({ event }: { event: EventRow }) {
  const title = encodeURIComponent(event.title);
  const startDate = parseISO(event.start_date);
  const endDate = event.end_date ? parseISO(event.end_date) : new Date(startDate.getTime() + 60 * 60 * 1000);
  const details = encodeURIComponent([event.notes, event.meeting_url].filter(Boolean).join('\n'));
  const location = event.meeting_url ? encodeURIComponent(event.meeting_url) : '';
  ...
  const downloadICS = () => {
    const ics = [
      ...
      event.meeting_url ? `LOCATION:${event.meeting_url}` : '',
      'END:VEVENT',
      'END:VCALENDAR',
    ].filter(Boolean).join('\r\n');
