        ...
        <Card className="overflow-hidden">
          <CardHeader className="pb-3 bg-muted/30 border-b">
            <div className="flex items-center gap-3">
              ...
              <Badge variant="outline" className="text-[10px] ml-auto">{participants.length}</Badge>
            </div>
          </CardHeader>
            ...
            {participants.length > 0 && (
              <div className="flex flex-wrap gap-2">
                {participants.map(p => {
                  const profile = profiles.find(pr => pr.id === p.profile_id);
                  return (
                    <div key={p.id} className="flex items-center gap-2 rounded-full border bg-muted/30 pl-1 pr-2.5 py-1 text-sm">
                      <Avatar className="h-6 w-6">
                        <AvatarImage src={profile?.avatar_url || ''} />
                        <AvatarFallback className="text-[9px]">{initials(profile?.full_name || null)}</AvatarFallback>
                      </Avatar>
