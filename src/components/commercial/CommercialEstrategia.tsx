...
import { Plus, FileDown, Check, Loader2 } from 'lucide-react';
...
export function CommercialEstrategia() {
  const { isOwner } = useAuth();
  const queryClient = useQueryClient();
  const [title, setTitle] = useState('Estratégia de Vendas');
  ...
  useEffect(() => {
    if (strategy) {
      ...
      setSections(DEFAULT_SECTIONS.map(d => ({ ...d, id: generateId() })));
    }
  }, [strategy, isLoading]);
  ...
  const save = useCallback(async (t: string, p: string, s: Section[]) => {
    ...
    try {
      if (strategy) {
        ...
        const { error } = await supabase
          .from('commercial_strategy')
          .insert({ title: t, period: p, sections: s as any } as any);
        if (error) throw error;
      }
