  ...
  return (
    <div className="space-y-6 mt-4">
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
        <Card><CardContent className="pt-4"><p className="text-xs text-muted-foreground">Total Entradas</p><p className="text-xl font-bold text-emerald-600">{fmt(totals.entradas)}</p></CardContent></Card>
        <Card><CardContent className="pt-4"><p className="text-xs text-muted-foreground">Total Saídas</p><p className="text-xl font-bold text-red-600">{fmt(totals.saidas)}</p></CardContent></Card>
        <Card><CardContent className="pt-4"><p className="text-xs text-muted-foreground">Resultado Anual</p><p className={`text-xl font-bold ${totals.resultado >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{fmt(totals.resultado)}</p></CardContent></Card>
        <Card><CardContent className="pt-4"><p className="text-xs text-muted-foreground">Margem Anual</p><p className={`text-xl font-bold ${totals.margem >= 0 ? 'text-emerald-600' : 'text-red-600'}`}>{totals.margem}%</p></CardContent></Card>
      </div>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <Card className="border-emerald-200 bg-emerald-50/50 dark:bg-emerald-950/20 dark:border-emerald-800">
          <CardContent className="pt-4 flex items-center gap-3">
            ...
            <div>
              ...
              <p className="font-semibold">{bestQuarter?.label} ({bestQuarter?.range})</p>
              <p className="text-sm text-emerald-600">{fmt(bestQuarter?.resultado || 0)}</p>
            </div>
          </CardContent>
        </Card>
        <Card className="border-red-200 bg-red-50/50 dark:bg-red-950/20 dark:border-red-800">
          <CardContent className="pt-4 flex items-center gap-3">
            ...
            <div>
              <p className="text-xs text-muted-foreground">Pior trimestre</p>
              <p className="font-semibold">{worstQuarter?.label} ({worstQuarter?.range})</p>
              <p className="text-sm text-red-600">{fmt(worstQuarter?.resultado || 0)}</p>
            </div>
          </CardContent>
        </Card>
      </div>
          ...
          <Table>
            ...
            <TableBody>
              ...
              <TableRow className="bg-muted/50 font-semibold">
                ...
                <TableCell className="text-right">{totals.margem}%</TableCell>
                <TableCell></TableCell>
                <TableCell></TableCell>
              </TableRow>
            </TableBody>
      ...
      {/* Distribution: Products & Categories side by side */}
      <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-1.5"><Package className="h-3.5 w-3.5" /> Distribuição por Produto</CardTitle>
          </CardHeader>
          <CardContent>
            {allProducts.length > 0 ? (
              <div className="flex items-center gap-4">
                <div className="h-48 w-48 shrink-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={allProducts} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} strokeWidth={1}>
                        {allProducts.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                      </Pie>
                      <Tooltip formatter={(v: number) => fmt(v)} />
                    </PieChart>
                  </ResponsiveContainer>
        ...
        <Card>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm flex items-center gap-1.5"><ArrowUpRight className="h-3.5 w-3.5" /> Distribuição por Categoria de Despesa</CardTitle>
          </CardHeader>
          <CardContent>
            {allCategories.length > 0 ? (
              <div className="flex items-center gap-4">
                <div className="h-48 w-48 shrink-0">
                  <ResponsiveContainer width="100%" height="100%">
                    <PieChart>
                      <Pie data={allCategories} dataKey="value" nameKey="name" cx="50%" cy="50%" outerRadius={70} strokeWidth={1}>
                        {allCategories.map((_, i) => <Cell key={i} fill={PIE_COLORS[i % PIE_COLORS.length]} />)}
                      </Pie>
                      <Tooltip formatter={(v: number) => fmt(v)} />
                    </PieChart>
                  </ResponsiveContainer>
          ...
          <Table>
            <TableHeader>
              <TableRow>
                ...
                <TableHead className="text-right">Balanço IVA</TableHead>
      ...
      {/* Per-quarter detailed breakdown */}
      {data.map(d => (
        <Card key={d.label}>
          <CardHeader className="pb-2">
            <CardTitle className="text-sm">{d.label} — {d.range} — Detalhe</CardTitle>
          </CardHeader>
          <CardContent>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              {d.products.length > 0 && (
                <div>
                  <p className="text-xs text-muted-foreground mb-2">Receita por produto</p>
                  <div className="space-y-1">
                    {d.products.map(p => (
                      <div key={p.name} className="flex justify-between text-sm">
                        <span className="truncate">{p.name}</span>
                        <span className="text-muted-foreground shrink-0 ml-2">{fmt(p.value)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                  ...
                  <div className="space-y-1">
                    {d.categories.map(c => (
                      <div key={c.name} className="flex justify-between text-sm">
                        ...
                        <span className="text-muted-foreground shrink-0 ml-2">{fmt(c.value)}</span>
                      </div>
                    ))}
                  </div>
                </div>
