            ...
            <TableBody>
              ...
              {projectTemplate.map((t) => (
                <TableRow key={t.id as string}>
                  <TableCell><Input defaultValue={(t.phase as string) || ''} placeholder="Fase" onBlur={e => onUpdateRow('product_project_templates', t.id as string, { phase: e.target.value })} className="border-none shadow-none h-auto p-0 text-sm" readOnly={!isOwner} /></TableCell>
                  <TableCell><Input defaultValue={(t.task_name as string) || ''} placeholder="Nome da tarefa" onBlur={e => onUpdateRow('product_project_templates', t.id as string, { task_name: e.target.value })} className="border-none shadow-none h-auto p-0 text-sm" readOnly={!isOwner} /></TableCell>
                  <TableCell><Input defaultValue={(t.responsible as string) || ''} placeholder="Responsável" onBlur={e => onUpdateRow('product_project_templates', t.id as string, { responsible: e.target.value })} className="border-none shadow-none h-auto p-0 text-sm" readOnly={!isOwner} /></TableCell>
                  {isOwner && (
