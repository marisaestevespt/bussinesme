...
export function MonthDetailView({ monthIdx, year, planning, onBack }: Props) {
  ...
  const productsQ = useQuery({ queryKey: ['md-products'], queryFn: async () => { const { data } = await supabase.from('products').select('id, name, monthly_hours_per_client, base_price'); return data || []; }});
