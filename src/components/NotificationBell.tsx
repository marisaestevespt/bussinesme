              ...
              {notifications.map(n => (
                ...
                >
                  ...
                  <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity shrink-0">
                    {!n.read && (
                      <button onClick={e => { e.stopPropagation(); markAsRead.mutate(n.id); }} title="Marcar como lida">
                        <Check className="h-3.5 w-3.5 text-muted-foreground hover:text-primary" />
                      </button>
                    )}
                    <button onClick={e => { e.stopPropagation(); deleteNotification.mutate(n.id); }} title="Remover">
