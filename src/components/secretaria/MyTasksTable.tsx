...
import { format, parseISO, startOfDay, isWithinInterval, startOfWeek, endOfWeek, isToday, isBefore, parseISO as parse } from 'date-fns';
...
interface Props {
  /** Janela temporal a aplicar antes dos filtros To Do/Atrasadas/Concluídas. */
  scope?: Scope;
}
...
function inScope(t: any, scope: Scope): boolean {
  ...
  // Sempre incluir tarefas atrasadas em "today" e "week" (são responsabilidade imediata)
  if (t.deadline) {
...
export function MyTasksTable({ scope = 'all' }: Props) {
  ...
  const filtered = useMemo(() => {
    ...
    switch (view) {
      case 'todo': return scoped.filter(isTaskOpen).sort(byDeadlineAsc);
      case 'atrasadas': return scoped.filter((t: any) => isTaskOverdue(t, today)).sort(byDeadlineAsc);
      case 'concluidas': return scoped.filter(isTaskDone).sort((a: any, b: any) => (b.updated_at || '').localeCompare(a.updated_at || ''));
