...
BEGIN
  ...
  UPDATE financial_expenses SET status = 'tudo_ok' WHERE id = _expense_id;
  SELECT status INTO _tmp_status FROM member_payments WHERE id = _payment_id;
  RETURN QUERY SELECT 'T2: Status despesa->pagamento'::text, 'tudo_ok'::text, _tmp_status, _tmp_status = 'tudo_ok';
  ...
  RETURN QUERY SELECT 'T7: Re-ligacao ao set member_id'::text, 'true'::text, _exists::text, _exists;
