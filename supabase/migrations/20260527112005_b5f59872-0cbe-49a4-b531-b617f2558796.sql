-- Deliverable → Task: propagate assigned_to changes (not just first-time fill).
...
BEGIN
  ...
  ELSE
    ...
    SET name = NEW.name,
        ...
        -- If deliverable's assigned_to changed, push it through; otherwise keep task assignee or resolve.
        assigned_to = CASE
...
-- Task → Deliverable: also propagate assigned_to.
...
BEGIN
  ...
  _assignee_changed := NEW.assigned_to IS DISTINCT FROM OLD.assigned_to;

  IF NOT (_status_changed OR _deadline_changed OR _name_changed OR _assignee_changed) THEN
    RETURN NEW;
  END IF;
  ...
  UPDATE public.project_deliverables
    SET status = COALESCE(_new_deliv_status, status),
        ...
        assigned_to = CASE WHEN _assignee_changed THEN NEW.assigned_to ELSE assigned_to END,
        updated_at = now()
