...
-- #15 + #12: harden activate_renewal_project (validate product) + audit
...
BEGIN
  ...
  -- #15: validate product still exists
  ...
  THEN
    ...
    SELECT _proj.client_id, _next_cycle, t.name, t.notes, t.responsible_type,
  ...
  -- #12: audit (only when called by an authenticated user; skip when service_role/cron)
...
-- #14: cascade-delete unpaid sales when scheduled project is deleted OR its dates change
CREATE OR REPLACE FUNCTION public.cleanup_sales_on_scheduled_project_change()
RETURNS trigger
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path TO 'public'
...
BEGIN
  IF TG_OP = 'DELETE' THEN
    IF OLD.status = 'agendado' THEN
      ...
      WHERE project_id = OLD.id AND status <> 'pago';
    END IF;
    RETURN OLD;
  END IF;

  IF TG_OP = 'UPDATE' AND OLD.status = 'agendado' AND NEW.status = 'agendado' THEN
    IF OLD.start_date IS DISTINCT FROM NEW.start_date
       OR OLD.deadline IS DISTINCT FROM NEW.deadline THEN
      DELETE FROM public.commercial_sales
      WHERE project_id = NEW.id AND status <> 'pago';
    END IF;
  END IF;
  RETURN NEW;
END;
...
DROP TRIGGER IF EXISTS cleanup_sales_on_scheduled_project_change_trg ON public.projects;
CREATE TRIGGER cleanup_sales_on_scheduled_project_change_trg
AFTER UPDATE OR DELETE ON public.projects
FOR EACH ROW EXECUTE FUNCTION public.cleanup_sales_on_scheduled_project_change();
