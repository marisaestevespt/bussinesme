...
CREATE OR REPLACE FUNCTION public.get_portal_meetings(_token uuid)
 RETURNS TABLE(id uuid, title text, date_time timestamp with time zone, status text, meeting_url text, department text, client_id uuid, project_id uuid, project_name text, portal_notes text, duration_minutes integer)
 LANGUAGE sql
