...
BEGIN
  ...
  -- Cascade phase dates from project.start_date
  IF _project_start IS NOT NULL THEN
    ...
    LOOP
      ...
      -- Only set if not already set (preserve manual edits)
      ...
      -- Set deliverable dates = phase end (only if NULL)
    ...
    -- Auto-create meetings for deliverables marked is_meeting
    FOR _del IN
    ...
    LOOP
      INSERT INTO public.meetings (
        ...
        _project_id, _project_client, _project_dept, 'agendada', 'cliente'
  ...
  -- Generate cycle occurrences (recurring services)
