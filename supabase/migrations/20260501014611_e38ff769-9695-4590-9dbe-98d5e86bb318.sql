...
SET search_path = public
AS $$
  SELECT cp.id, cp.client_id, cp.is_active, cp.show_onboarding, cp.show_timeline, cp.show_payments, cp.show_meetings, cp.show_faqs, cp.show_monthly_summary, cp.show_workspace, cp.portal_type, cp.slug
  FROM public.client_portals cp
